...
class M_order extends CI_Model
{
	...
	{
		$data['deliv'] = $this->db->query("SELECT * FROM `transaksi` JOIN pelanggan ON transaksi.id_pelanggan=pelanggan.id_pelanggan JOIN pengiriman ON transaksi.id_transaksi=pengiriman.id_transaksi WHERE type_order='1' AND pelanggan.id_pelanggan=" . $this->session->userdata('id_pelanggan'))->result();
		$data['dinein'] = $this->db->query("SELECT * FROM `transaksi` JOIN pelanggan ON transaksi.id_pelanggan=pelanggan.id_pelanggan WHERE type_order='2' AND pelanggan.id_pelanggan=" . $this->session->userdata('id_pelanggan'))->result();
		return $data;
	}
